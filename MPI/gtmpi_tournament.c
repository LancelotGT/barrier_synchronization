...
void gtmpi_init(int num_threads){
	...
	for (k = 0; k <= num_rounds; k++) {
		...
		// opponent 
		if (rounds[i][k].role == LOSER) //rounds[i][k].opponent = &rounds[i - (1 << k - 1)][k].flag;
			rounds[i][k].opponent = i - (1 << (k - 1));
		else if (rounds[i][k].role == WINNER || rounds[i][k].role == CHAMPION) {
			//rounds[i][k].opponent = &rounds[i + (1 << k - 1)][k].flag;
			rounds[i][k].opponent = i + (1 << (k - 1));
		}
	}
...
void gtmpi_barrier(){
	...
	int i;
	if (vpid == 0) {
		for (k = num_rounds; k >= 0; k--) {
			for (i = 0; i < P; i++) {
				printf("P%d: %d ", i, rounds[i][k].role);
			}
			printf("\n");
		}
	}
	*/
  	...
  	for (k = num_rounds - 1; k >= 0; k--) {
   		if (rounds[vpid][k].role == WINNER) //*rounds[vpid][i].opponent = sense;
   			MPI_Send(NULL, 0, MPI_INT, rounds[vpid][k].opponent, 1, MPI_COMM_WORLD);
  	}
}
